...
using TownOfSushi.CrewmateRoles.SwapperMod;
using TownOfSushi.CrewmateRoles.VigilanteMod;
using TownOfSushi.NeutralRoles.DoomsayerMod;
using TownOfSushi.CultistRoles.NecromancerMod;
using TownOfSushi.CustomOption;
using TownOfSushi.Extensions;
using TownOfSushi.Modifiers.AssassinMod;
using TownOfSushi.NeutralRoles.ExecutionerMod;
using TownOfSushi.NeutralRoles.GuardianAngelMod;
using TownOfSushi.ImpostorRoles.MinerMod;
...
namespace TownOfSushi
{
    public static class RpcHandling
    {
        ...
        private static void GenEachRole(List<GameData.PlayerInfo> infected)
        {
            ...
            canHaveAssassinModifier.RemoveAll(player => !player.Is(AbilityEnum.Assassin)||!player.Is(RoleEnum.Vigilante));
            ...
            var toChooseFromCrew = PlayerControl.AllPlayerControls.ToArray().Where(x => x.Is(Faction.Crewmates) && !x.Is(RoleEnum.Monarch)).ToList();
            ...
            var toChooseFromNeut = PlayerControl.AllPlayerControls.ToArray().Where(x => (x.Is(Faction.NeutralBenign) || x.Is(Faction.NeutralEvil) || x.Is(Faction.NeutralKilling))).ToList();
            ...
            var exeTargets = PlayerControl.AllPlayerControls.ToArray().Where(x => x.Is(Faction.Crewmates) && !x.Is(ModifierEnum.Lover) && !x.Is(RoleEnum.Monarch) && !x.Is(RoleEnum.Swapper) && !x.Is(RoleEnum.Vigilante) && !x.Is(RoleEnum.Mayor) && x != SetTraitor.WillBeTraitor).ToList();
        ...
        private static void GenEachRoleCultist(List<GameData.PlayerInfo> infected)
        {
            ...
            if (CustomGameOptions.MonarchCultistOn > 0) specialRoles.Add((typeof(Monarch), CustomGameOptions.MonarchCultistOn, true));
            if (CustomGameOptions.SnitchCultistOn > 0) specialRoles.Add((typeof(CultistSnitch), CustomGameOptions.SnitchCultistOn, true));
            ...
            if (specialRoles.Count < crewmates.Count)
            {
                ...
                var snitches = CustomGameOptions.MaxInformantes;
                ...
                while (snitches > 0)
                {
                    crewRoles.Add((typeof(CultistInformant), 10, false));
                    snitches--;
                }
                while (spies > 0)
                {
            ...
            public static void Postfix([HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
            {
                ...
                switch ((CustomRPC) callId)
                {
                    ...
                    case CustomRPC.Reveal:
                        var monarch = Utils.PlayerById(reader.ReadByte());
                        var monarchRole = Role.GetRole<Monarch>(monarch);
                        monarchRole.Revealed = true;
                        AddRevealButton.RemoveAssassin(monarchRole);
                        break;
                    ...
                    case CustomRPC.Blackmail:
                        ...
                        break;
                    case CustomRPC.InformantCultistReveal:
                        var snitch = Role.GetRole<CultistInformant>(Utils.PlayerById(reader.ReadByte()));
                    ...
                    case CustomRPC.AltruistRevive:
                        ...
                        foreach (var body in theDeadBodies)
                            if (body.ParentId == readByte)
                            {
                                ...
                                Coroutines.Start(
                                    global::TownOfSushi.CrewmateRoles.AltruistMod.Coroutine.AltruistRevive(body,
                                        altruistRole));
                            }
                    ...
                    case CustomRPC.CatchAvenger:
                        ...
                        Role.GetRole<Avenger>(haunterPlayer).Caught = true;
            ...
            public static void Postfix()
            {
                ...
                if (CustomGameOptions.GameMode == GameMode.Classic || CustomGameOptions.GameMode == GameMode.AllAny)
                {
                    ...
                    if (CustomGameOptions.MonarchOn > 0)
                        CrewmateRoles.Add((typeof(Monarch), CustomGameOptions.MonarchOn, true));
                    ...
                    if (CustomGameOptions.SnitchOn > 0)
                        CrewmateRoles.Add((typeof(Snitch), CustomGameOptions.SnitchOn, false));

                    if (CustomGameOptions.InformantOn > 0)
                        CrewmateRoles.Add((typeof(Informant), CustomGameOptions.InformantOn, true));
                    ...
                    if (Check(CustomGameOptions.LighterOn))
                        CrewmateModifiers.Add((typeof(Lighter), CustomGameOptions.LighterOn));

                    if (Check(CustomGameOptions.BaitOn))
                        GlobalModifiers.Add((typeof(Bait), CustomGameOptions.BaitOn));                        
                    ...
                    if (Check(CustomGameOptions.ParanoiacOn))
                        GlobalModifiers.Add((typeof(Paranoiac), CustomGameOptions.ParanoiacOn));
                    #endregion
