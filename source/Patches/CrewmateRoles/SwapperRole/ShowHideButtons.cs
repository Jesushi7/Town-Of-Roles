...
using TownOfRoles.CrewmateRoles.MayorRole;
...
namespace TownOfRoles.CrewmateRoles.SwapperRole
{
    public class ShowHideButtons
    {
        public static Dictionary<byte, int> CalculateVotes(MeetingHud __instance)
        {
            ...
            PluginSingleton<TownOfRoles>.Instance.Log.LogInfo($"Swap1 playerid = {SwapVotes.Swap1.TargetPlayerId}");
            var swap1 = 0;
            if (self.TryGetValue(SwapVotes.Swap1.TargetPlayerId, out var value)) swap1 = value;
            PluginSingleton<TownOfRoles>.Instance.Log.LogInfo($"Swap1 player has votes = {swap1}");

            var swap2 = 0;
            PluginSingleton<TownOfRoles>.Instance.Log.LogInfo($"Swap2 playerid = {SwapVotes.Swap2.TargetPlayerId}");
            if (self.TryGetValue(SwapVotes.Swap2.TargetPlayerId, out var value2)) swap2 = value2;
            PluginSingleton<TownOfRoles>.Instance.Log.LogInfo($"Swap2 player has votes  = {swap2}");
        ...
        public static class CheckForEndVoting
        {
            ...
            public static bool Prefix(MeetingHud __instance)
            {
                if (__instance.playerStates.All(ps => ps.AmDead || ps.DidVote && CheckVoted(ps)))
                {
                    ...
                    PluginSingleton<TownOfRoles>.Instance.Log.LogMessage($"Meeting was a tie = {tie}");
