...
namespace TownOfRoles.CrewmateRoles.GamblerMod
{
    ...
    public class AddButton
    {
        ...
        public static void GenButton(Gambler role, PlayerVoteArea voteArea)
        {
        ...
        private static Action Cycle(Gambler role, PlayerVoteArea voteArea, TextMeshPro nameText, bool forwardsCycle = true)
        {
            void Listener()
            {
        ...
        private static Action Guess(Gambler role, PlayerVoteArea voteArea)
        {
            void Listener()
            {
                if (
                ...
                {
                    GamblerKill.RpcMurderPlayer(toDie, PlayerControl.LocalPlayer);
                    role.RemainingKills--;
                    ShowHideButtonsGambler.HideSingle(role, targetId, toDie == role.Player);
                    if (toDie.IsLover() && CustomGameOptions.BothLoversDie)
                    {
                        var lover = ((Lover)playerModifier).OtherLover.Player;
                        if (!lover.Is(RoleEnum.Pestilence)) ShowHideButtonsGambler.HideSingle(role, lover.PlayerId, false);
                    }
                }
            }
        ...
        {
            foreach (var role in Role.GetRoles(RoleEnum.Gambler))
            {
                var retributionist = (Gambler)role;
            ...
            if (!PlayerControl.LocalPlayer.Is(RoleEnum.Gambler)) return;

            var retributionistRole = Role.GetRole<Gambler>(PlayerControl.LocalPlayer);
