...
namespace TownOfSushi.CrewmateRoles.MonarchMod
{
    ...
    public class AddRevealButton
    {
        ...
        public static void GenButton(Monarch role, int index)
        {
        ...
        private static Action Reveal(Monarch role)
        {
            void Listener()
            {
        ...
        public static void RemoveAssassin(Monarch mayor)
        {
        ...
        {
            foreach (var role in Role.GetRoles(RoleEnum.Monarch))
            {
                var mayor = (Monarch)role;
                mayor.RevealButton.Destroy();
            }

            if (PlayerControl.LocalPlayer.Data.IsDead) return;
            if (!PlayerControl.LocalPlayer.Is(RoleEnum.Monarch)) return;
            var mayorrole = Role.GetRole<Monarch>(PlayerControl.LocalPlayer);
