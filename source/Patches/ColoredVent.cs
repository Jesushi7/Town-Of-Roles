...
namespace TownOfRoles
{
    ...
    class SetVentOutlinePatch
    {
        public static void Postfix(Vent __instance, [HarmonyArgument(1)] ref bool mainTarget)
        {
            ...
            || player.Is(RoleEnum.Engineer) || player.Is(RoleEnum.Glitch) 
            || player.Is(RoleEnum.Jester) || player.Is(RoleEnum.Juggernaut) 
            || player.Is(RoleEnum.Pestilence) || player.Is(RoleEnum.Werewolf));
            if (!active) return;
